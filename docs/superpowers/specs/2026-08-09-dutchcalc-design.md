...
ステップ 3 の逐次貪欲法により、`sum(units) === totalUnits` が常に成立し、かつ払戻の最大・最小差が小さくなるように配分される。

※この貪欲法は「差(spread)が真に最小になる」ことを数学的に証明したものではないヒューリスティックである。妥当性はテスト方針の全探索比較テストで担保する。
...
- 候補数が少ない場合（2〜4 件程度）は、全探索（ブルートフォース）で求めた最小 `spread` の配分と一致することを確認する（貪欲法の妥当性検証）
