 ...
 * 動詞は活用形ごとに次の方針で展開する:
 *   四段:      語幹 3文字以上なら語幹で全形をカバー。2文字以下は語幹が短すぎて誤検索の恐れがあるため終止形のみ。
 *   下二段:    終止形(u段) + 連用/未然形(e段) の2形を生成（語幹が短い場合も正確にカバー）。
...
function getSearchTerms(word, pos) {
    ...
    // 四段・その他: 語幹 3文字以上なら語幹、それ以下は終止形のみ
...
function parseArgs(argv) {
    ...
    return { words, ids, sourceLabels, limit };
}
...
const { words, ids, sourceLabels, limit } = parseArgs(process.argv);
